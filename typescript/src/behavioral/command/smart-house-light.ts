export class SmartHouseLight {
  private isOn = false;
  private intensity = 50;

  constructor(public name: string) {}

  private consoleLogStatus() {
    console.log(`[STATUS] ${this.name}: ${this.getPowerStatus()}`);
  }

  private consoleLogIntensity() {
    console.log(`[INTENSITY] ${this.name}: ${this.intensity}`);
  }

  getPowerStatus(): string {
    return this.isOn ? 'ON' : 'OFF';
  }

  on(): boolean {
    this.isOn = true;
    this.consoleLogStatus();
    return this.isOn;
  }

  off(): boolean {
    this.isOn = false;
    this.consoleLogStatus();
    return this.isOn;
  }

  increaseIntensity(): number {
    if (this.intensity < 100) this.intensity += 1;

    this.consoleLogIntensity();

    return this.intensity;
  }

  decreaseIntensity(): number {
    if (this.intensity > 0) this.intensity -= 1;

    this.consoleLogIntensity();

    return this.intensity;
  }
}
